...
// 基座隔离：产品线可通过 LINGXIAO_HOME 覆盖配置/会话根目录，实现多实例隔离；
// 未设置时保持历史默认 ~/.lingxiao，行为不变。
export const CONFIG_DIR = resolve(
  process.env.LINGXIAO_HOME && process.env.LINGXIAO_HOME.trim()
    ? process.env.LINGXIAO_HOME.trim()
    : join(homedir(), '.lingxiao'),
);
