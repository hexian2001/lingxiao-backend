...
import { AssetUsageStore } from '@lingxiao-office/sdk/memory/AssetUsageStore.js';
import type { EventEmitter } from '@lingxiao-office/sdk/core/EventEmitter.js';
import type { DatabaseManager, ScheduledTaskRecord } from '@lingxiao-office/sdk/core/Database.js';
...
import { buildRuntimeDiagnosticsItems, buildRuntimeDiagnosticsPayload } from '@lingxiao-office/sdk/core/RuntimeDiagnostics.js';
import { ProjectRuntimeManager } from '@lingxiao-office/sdk/core/ProjectRuntimeManager.js';
import { EternalRuntimeTelemetry } from '@lingxiao-office/sdk/core/EternalRuntimeTelemetry.js';
import { ProjectRetentionPolicy } from '@lingxiao-office/sdk/core/ProjectRetentionPolicy.js';
import { BlockedAgingPolicy } from '@lingxiao-office/sdk/contracts/adapters/BlockedAgingPolicy.js';
import { detectSandboxCapabilities } from '@lingxiao-office/sdk/tools/implementations/ExecutionSandbox.js';
import {
  applyAndPersistPermissionUpdates,
  buildPermissionSurfaceItems,
  type PermissionUpdateDestination,
} from '@lingxiao-office/sdk/core/PermissionStore.js';
import { normalizeToolPermissionContext, summarizePermissionContextForDisplay } from '@lingxiao-office/sdk/core/PermissionSystem.js';
import {
  isProjectBacklogTerminalStatus,
  isProjectDependencyTerminalStatus,
  normalizeProjectRuntimeMode,
} from '@lingxiao-office/sdk/core/StateSemantics.js';
import { buildRoleSkillSurfaceItems, buildSkillSurfaceItems } from '@lingxiao-office/sdk/core/SkillCatalog.js';
import { getSessionScopeDescription } from '@lingxiao-office/sdk/tools/implementations/utils.js';
...
import { ProjectControlService } from '@lingxiao-office/sdk/agents/ProjectControlService.js';
import { buildBughuntRequest } from './bughunt.js';
import { findCustomCommand, renderCommandBody } from './CustomCommandLoader.js';
import { appendBughuntEvent, buildBughuntBrief, generateBughuntReport, readBughuntLedger, startOrResumeBughuntLedger } from '@lingxiao-office/sdk/core/BughuntLedger.js';
import { getModelManager } from '@lingxiao-office/sdk/config/ModelManager.js';
import { config as runtimeConfig, setConfigValue, saveSettings, ConfigSchema } from '@lingxiao-office/sdk/config.js';
import {
  t,
  setSessionLanguage,
  getSessionLanguage,
  normalizeLanguage,
} from '@lingxiao-office/sdk/i18n.js';
import {
  OFFICE_TOOL_NAMES,
  WORKFLOW_TOOL_NAMES,
} from '@lingxiao-office/sdk/contracts/constants/leaderToolDefinitions.js';
import {
...
} from '@lingxiao-office/sdk/contracts/types/Autonomy.js';
...
export interface CallbackCommandDispatcherContext {
  ...
  scheduledTaskManager?: import('@lingxiao-office/sdk/core/ScheduledTaskManager.js').ScheduledTaskManager;
}
...
async function handleCostCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  const { currentSessionId, db } = ctx;
  const { calculateSessionCost, formatCostReport } = await import('@lingxiao-office/sdk/llm/CostService.js');
  if (!currentSessionId) {
...
async function handleBugCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  ...
  const { buildDiagnosticsBundle } = await import('@lingxiao-office/sdk/core/Diagnostics.js');
  const { redactSensitiveString } = await import('@lingxiao-office/sdk/core/CrashReporter.js');
  const { VERSION } = await import('@lingxiao-office/sdk/version.js');
...
async function handleRewindCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  ...
  const { DatabaseRepositoryAdapter } = await import('@lingxiao-office/sdk/core/DatabaseRepositories.js');
...
async function handleWikiCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  ...
  const { createLLMClient } = await import('@lingxiao-office/sdk/llm/Client.js');
...
async function handleContractCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  ...
  if (subCommand === 'audit' && args[1] === 'generate') {
    ...
    const { config } = await import('@lingxiao-office/sdk/config.js');
  ...
  if (subCommand === 'audit' && args[1] === 'verify') {
    ...
    const { ContractAuditGenerator } = await import('../wiki/ContractAuditGenerator.js');
    const { config } = await import('@lingxiao-office/sdk/config.js');
    const { loadProjectContractEntries } = await import('@lingxiao-office/sdk/core/ProjectContracts.js');
    const { computeContractDrift, renderContractDriftReport } = await import('@lingxiao-office/sdk/core/ContractDriftAudit.js');
    const model = config.llm.wiki_model || config.llm.leader_model;
    if (!model) {
...
async function handleDreamCommand(ctx: CommandHandlerContext): Promise<CommandResult> {
  ...
  const workspacePath = currentSessionId
    ? (db.getSession(currentSessionId)?.workspace || cwd)
    : cwd;
  const { runMemoryMaintenancePipeline } = await import('@lingxiao-office/sdk/memory/MemoryMaintenanceStatus.js');
  try {
