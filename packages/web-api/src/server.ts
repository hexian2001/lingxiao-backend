...
import { DatabaseManager } from '@lingxiao-office/sdk/core/Database.js';
import { SessionManager } from '@lingxiao-office/sdk/core/SessionManager.js';
import { DatabaseRepositoryAdapter } from '@lingxiao-office/sdk/core/DatabaseRepositories.js';
import { createEventEmitter, createMessageBus, registerCleanup, gracefulShutdown } from '@lingxiao-office/sdk/core/index.js';
import { WorkerProcessRunner } from '@lingxiao-office/sdk/core/WorkerProcessRunner.js';
import { coreLogger } from '@lingxiao-office/sdk/core/Log.js';
import { ScheduledTaskManager } from '@lingxiao-office/sdk/core/ScheduledTaskManager.js';
import { ResourceBudgetService } from '@lingxiao-office/sdk/core/ResourceBudgetService.js';
import { UpdateChecker } from '@lingxiao-office/sdk/core/UpdateChecker.js';
import { tempDownloadRegistry } from '@lingxiao-office/sdk/core/TempDownloadRegistry.js';
import { config as runtimeConfig, getConfigValue, startSettingsWatcher, stopSettingsWatcher } from '@lingxiao-office/sdk/config.js';
import { installProcessRuntimeGuards } from '@lingxiao-office/sdk/core/RuntimeGuards.js';
...
import { initLangfuse, shutdownLangfuse, readLangfuseConfig, setLangfuseEmitter } from '@lingxiao-office/sdk/core/LangfuseIntegration.js';
import { onConfigReload } from '@lingxiao-office/sdk/config.js';
import { reconcileToolRegistryFromConfig } from '@lingxiao-office/sdk/tools/index.js';
...
import { createLlmGuard } from '@lingxiao-office/sdk/agents/LlmGuard.js';
import { getExternalAgentAvailability } from '@lingxiao-office/sdk/agents/external/availability.js';

const defaultEmitter = createEventEmitter();
const defaultMessageBus = createMessageBus(1000, defaultEmitter);
import { startLocalLlmGatewayServer } from '@lingxiao-office/sdk/core/LocalLlmGatewayServer.js';
import { BrowserRuntime } from '@lingxiao-office/sdk/core/BrowserRuntime.js';
import { ActiveSessionCoordinator } from '@lingxiao-office/sdk/core/ActiveSessionCoordinator.js';
import ws from '@fastify/websocket';
import { FILE_PARSER } from '@lingxiao-office/sdk/config/defaults.js';
import { configureWebLogging, ensureLogMaintenance } from '@lingxiao-office/sdk/runtime/LoggingRuntime.js';
import { writeWatchdog } from '@lingxiao-office/sdk/core/EternalSupervisor.js';
import { rateLimitExemptLocalhost, isHardenedMode } from '@lingxiao-office/sdk/core/HardeningPolicy.js';
import { processExists } from '@lingxiao-office/sdk/utils/platform.js';
import { WebApiRouteRegistry, type WebApiExtension, type WebApiExtensionContext } from './web-server/ExtensionRegistry.js';
...
export interface CreateServerWithDepsOptions {
  logger?: boolean;
  ...
  /** Ordered Web API extensions registered after core API routes and before SPA fallback. */
  extensions?: readonly WebApiExtension[];
}
...
export async function createServerWithDeps(
  ...
  options?: CreateServerWithDepsOptions,
) {
  ...
  try {
    ...
    onConfigReload((cfg) => {
      ...
      import('@lingxiao-office/sdk/core/LangfuseIntegration.js').then(({ langfuseIntegration }) => {
  ...
  // ============================================================
  // 外部扩展路由注册（在核心 API 后、SPA fallback 前）
  // ============================================================
  if (options?.extensions?.length) {
    const extensionContext: WebApiExtensionContext = {
      fastify,
      requireServerToken,
      sessionManager,
      repos,
    ...
    const extensionRegistry = new WebApiRouteRegistry(options.extensions);
    await extensionRegistry.registerAll(extensionContext);
  }
...
export async function startServer() {
  ...
  const zombieScanTimer = setInterval(async () => {
    try {
      ...
      const { scanZombieProcesses } = await import('@lingxiao-office/sdk/tools/implementations/Shell.js');
      scanZombieProcesses();
    } catch { /* 静默失败 */ }
  }, 60_000);
