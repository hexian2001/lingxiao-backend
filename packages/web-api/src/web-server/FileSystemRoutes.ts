...
import { Workspace } from '@lingxiao-office/sdk/core/Workspace.js';
import type { AuthFn } from './types.js';
import { serverLogger } from '@lingxiao-office/sdk/core/Log.js';
...
export function registerFileSystemRoutes(
  ...
  fastify.post('/api/v1/files/upload', async (request, reply) => {
    ...
    try {
      ...
      const { parsePreview } = await import('@lingxiao-office/sdk/tools/implementations/FileParser.js');
  ...
  fastify.post('/api/v1/files/parse', async (request, reply) => {
    ...
    try {
      const { parseFile } = await import('@lingxiao-office/sdk/tools/implementations/FileParser.js');
      type ParseMode = import('@lingxiao-office/sdk/tools/implementations/FileParser.js').ParseMode;
