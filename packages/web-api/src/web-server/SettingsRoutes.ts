...
import type { DatabaseRepositoryAdapter } from '@lingxiao-office/sdk/core/DatabaseRepositories.js';
import type { SessionManager } from '@lingxiao-office/sdk/core/SessionManager.js';
import { SESSION_KEYS } from '@lingxiao-office/sdk/core/SessionStateKeys.js';
import { getConfigValue, setConfigValue, saveSettings, ConfigSchema, config as runtimeConfig, syncDerivedConstants, fireConfigReload, type ModelProvidersConfig, type ModelProviderConfig } from '@lingxiao-office/sdk/config.js';
import { getModelManager } from '@lingxiao-office/sdk/config/ModelManager.js';
import { isHardenedMode, isHardenedModeLocked } from '@lingxiao-office/sdk/core/HardeningPolicy.js';
import { LLM, TOOLS } from '@lingxiao-office/sdk/config/defaults.js';
import { toErrorMessage } from '@lingxiao-office/sdk/core/errors.js';
import { resolveLocalLlmGateway } from '@lingxiao-office/sdk/core/LocalLlmGateway.js';
import type { ContentGenerator } from '@lingxiao-office/sdk/llm/ContentGenerator.js';
import type { ChatMessage, ChatResponse, StreamCallbacks, ToolDefinition } from '@lingxiao-office/sdk/llm/types.js';
import type { LLMError } from '@lingxiao-office/sdk/llm/errors.js';
import { estimateTokens } from '@lingxiao-office/sdk/llm/token_counter.js';
import { DEFAULT_LINGXIAO_USER_AGENT, isLingxiaoDefaultUserAgent, normalizeLingxiaoUserAgent } from '@lingxiao-office/sdk/version.js';
...
const SETTINGS_MAP: Record<string, string> = {
  ...
  'autocompactRatio': 'context.autocompact_ratio',
  'contextTokenLimit': 'context.token_limit',
  'autocompactBufferTokens': 'context.autocompact_buffer_tokens',
  'compactLlmTimeoutMs': 'context.compact_llm_timeout_ms',
  'maxConsecutiveFailures': 'context.max_consecutive_failures',
  'maxRequestBytes': 'context.max_request_bytes',
  'maxSingleMessageBytes': 'context.max_single_message_bytes',
  'preservedSystemCount': 'context.preserved_system_count',
  'preservedRecentCount': 'context.preserved_recent_count',
  'maxRecentMessageCount': 'context.max_recent_message_count',
  'postCompactTokenBudget': 'context.post_compact_token_budget',
  'recentWindowTokenBudget': 'context.recent_window_token_budget',
...
export function registerSettingsRoutes(
  ...
  fastify.get('/api/v1/model/capabilities', async (request, reply) => {
    if (!requireServerToken(request, reply)) return;
    try {
      const { supportsVisionFromProvider, getContextWindowSizeFromProvider } = await import('@lingxiao-office/sdk/llm/model_capabilities.js');
      const { getModelDevInfo } = await import('@lingxiao-office/sdk/llm/ModelsDevRegistry.js');
      const { LLM } = await import('@lingxiao-office/sdk/config/defaults.js');
  ...
  fastify.get('/api/v1/model/registry/status', async (request, reply) => {
    if (!requireServerToken(request, reply)) return;
    try {
      const { getModelsDevRegistry } = await import('@lingxiao-office/sdk/llm/ModelsDevRegistry.js');
  ...
  fastify.post('/api/v1/prompt/enhance', async (request, reply) => {
    ...
    try {
      const { LLMClientManager } = await import('@lingxiao-office/sdk/llm/Client.js');
...
4. 不要加任何解释或前缀`;
  ...
  fastify.put('/api/v1/settings/:group', async (request, reply) => {
    ...
    try {
      ...
      if (configPath.startsWith('network.proxy.')) {
        const { rebuildSharedFetch } = await import('@lingxiao-office/sdk/llm/http_dispatcher.js');
        rebuildSharedFetch();
      }
      ...
      if (configPath === 'ui.language') {
        const { setLanguage, setSessionLanguage } = await import('@lingxiao-office/sdk/i18n.js');
      ...
      if (configPath === 'mcp.enabled' || configPath === 'mcp.tool_timeout_ms') {
        try {
          const { resetRuntimeMcpClient } = await import('@lingxiao-office/sdk/core/McpClient.js');
  ...
  fastify.get('/api/v1/prompts', async (request, reply) => {
    ...
    try {
      ...
      const { RESEARCH_SYSTEM_PROMPT_BY_LOCALE, EXPLORE_SYSTEM_PROMPT_BY_LOCALE, CODING_SYSTEM_PROMPT_BY_LOCALE, VERIFY_SYSTEM_PROMPT_BY_LOCALE, REVIEW_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/worker/system_prompts.js');
      const { FRONTEND_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/frontend_system.js');
      const { BACKEND_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/backend_system.js');
      const { FULLSTACK_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/fullstack_system.js');
      const { QA_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/qa_system.js');
      const { UX_DESIGNER_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/ux_designer_system.js');
      const { PLANNER_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/planner_system.js');
      const { EVALUATOR_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/evaluator_system.js');
      const { ARCHITECT_SYSTEM_PROMPT_BY_LOCALE } = await import('@lingxiao-office/sdk/agents/prompts/architect_system.js');
      const { getLeaderSystemPrompt } = await import('@lingxiao-office/sdk/agents/prompts/leader/system_prompt.js');
