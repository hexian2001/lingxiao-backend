...
import { isEmptyContent, type MessageContent } from '@lingxiao-office/sdk/llm/types.js';
import { config as runtimeConfig } from '@lingxiao-office/sdk/config.js';
...
export function registerSessionRoutes(
  ...
  fastify.get('/api/v1/sessions/:id/graph/analysis', async (request, reply) => {
    ...
    try {
      const { GraphStore } = await import('@lingxiao-office/sdk/core/blackboard/GraphStore.js');
      const { BlackboardGraph } = await import('@lingxiao-office/sdk/core/blackboard/BlackboardGraph.js');
      const { DispatcherEngine } = await import('@lingxiao-office/sdk/core/blackboard/DispatcherEngine.js');
