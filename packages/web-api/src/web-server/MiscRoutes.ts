...
import { metrics, refreshUptime } from '@lingxiao-office/sdk/core/MetricsRegistry.js';
import type { AuthFn } from './types.js';
import { VERSION } from '@lingxiao-office/sdk/version.js';
import { requireTokenInHeaderOnly, rateLimitExemptLocalhost } from '@lingxiao-office/sdk/core/HardeningPolicy.js';
...
async function fetchLatestRelease(): Promise<GitHubRelease | null> {
  // 基座可配置：产品线可通过 LINGXIAO_UPDATE_REPO=owner/repo 覆盖更新检查仓库；默认保留原仓库。
  const repo = (process.env.LINGXIAO_UPDATE_REPO && process.env.LINGXIAO_UPDATE_REPO.trim()) || 'hexian2001/lingxiao-coding';
